...
// ReSharper restore CheckNamespace
{
   ...
   public class SentryTarget : TargetWithLayout
   {
      ...
      /// <summary>
      /// Get sentry tags from event.
      /// </summary>
      /// <param name="logEvent">Logging event to be processed.</param>
      /// <returns>Sentry tags found in event.</returns>
      private Dictionary<string, string> GetTags(LogEventInfo logEvent)
      {
         if (logEvent == null || logEvent.Properties == null) return null;
         return logEvent.Properties
            .Where(w => w.Key is string && w.Value is string && ((string)w.Key).StartsWith(SentryLayoutRenderer.TAG_PREFIX))
            .ToDictionary(k => ((string)k.Key).Replace(SentryLayoutRenderer.TAG_PREFIX, ""), v => (string)v.Value);
      }
